...
export default function AddEditBrandModal({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = data
        ? await updateBrand(data.id, payload, imageFile || undefined)
        : await createBrand(payload, imageFile || undefined);
      ...
      setError(response.message || response.error?.message || "Failed to save brand");
    } catch (err) {
      setError(getErrorMessage(err, "An error occurred while saving brand"));
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Modal open={open} onClose={onClose} className="w-[900px] p-8 max-w-[95vw]">
      <h2 className="text-2xl font-bold mb-6">{data ? "Edit Brand" : "Add Brand"}</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div>
          ...
          >
            {image ? (
              <img src={image} alt="Brand" className="w-full h-48 object-cover rounded" />
            ) : (
              <>
