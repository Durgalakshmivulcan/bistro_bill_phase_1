import { useState, useEffect, useCallback } from "react";
import Pagination from "../../Common/Pagination";
import ReportActions from "../../Common/ReportActions";
import Select from "../../form/Select";
import { Search } from "lucide-react";
import { LoadingSpinner } from "../../Common";
import { getReservations, Reservation } from "../../../services/reservationService";
import { getBranches } from "../../../services/branchService";

const ReservationReport = () => {
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [reservations, setReservations] = useState<Reservation[]>([]);
  const [branchOptions, setBranchOptions] = useState<Array<{ label: string; value: string }>>([]);

  useEffect(() => {
    const loadBranches = async () => {
      try {
        const res = await getBranches({ status: "Active" });
        if (res.success && res.data) {
          setBranchOptions([
            { label: "Filter by Branch", value: "Filter by Branch" },
            ...res.data.branches.map((b) => ({ label: b.name, value: b.name })),
          ]);
        }
      } catch (err) {
        console.error("Failed to load branches:", err);
      }
    };
    loadBranches();
  }, []);

  const fetchReport = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await getReservations();
      if (response.success && response.data) {
        setReservations(response.data);
      } else {
        setError("Failed to load reservation report");
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to load reservation report");
    } finally {
      setLoading(false);
    }
  }, []);

  useEffect(() => {
    fetchReport();
  }, [fetchReport]);

  if (loading) {
    return (
      <div className="mx-2 sm:mx-4 mt-4">
        <LoadingSpinner message="Loading reservation report..." />
      </div>
    );
  }

  if (error) {
    return (
      <div className="mx-2 sm:mx-4 mt-4">
        <div className="bg-red-50 border border-red-200 rounded-lg p-4 text-center">
          <p className="text-red-600 mb-3">{error}</p>
          <button
            onClick={fetchReport}
            className="bg-bb-primary text-black px-4 py-2 rounded-md font-medium hover:bg-yellow-500"
          >
            Retry
          </button>
        </div>
      </div>
    );
  }

  const formatDate = (dateStr: string) => {
    const d = new Date(dateStr);
    return d.toLocaleDateString("en-GB");
  };

  const formatDateTime = (dateStr: string) => {
    const d = new Date(dateStr);
    return `${d.toLocaleDateString("en-GB")} ${d.toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" })}`;
  };

  const statusColors: Record<string, string> = {
    Pending: "bg-blue-100 text-blue-600",
    Confirmed: "bg-green-100 text-green-600",
    Cancelled: "bg-red-100 text-red-600",
    Completed: "bg-gray-200 text-gray-600",
  };

  return (
    <div className="mx-2 sm:mx-4 mt-4">

        <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
          <h3 className="text-xl font-semibold">
            Reservation Report
          </h3>

          <div className="flex flex-wrap gap-2">
            <div className="relative w-64">
              <Search
                size={16}
                className="absolute right-3 top-1/2 -translate-y-1/2 text-black"
              />
              <input
                placeholder="Search here..."
                className="w-full border border-gray-600 rounded-md px-3 pr-10 py-2 text-sm"
              />
            </div>

            <ReportActions />


          </div>
        </div>

        {/* ================= FILTER BAR ================= */}
        <div className="flex justify-end gap-3 mb-6 flex-wrap">

          <div className="w-48">
            <Select
              value="Filter by Date"
              options={[
                { label: "Filter by Date", value: "Filter by Date" },
                { label: "Today", value: "Today" },
                { label: "This Month", value: "This Month" },
              ]}
            />
          </div>

          <div className="w-48">
            <Select
              value="Filter by Branch"
              options={branchOptions.length > 0 ? branchOptions : [{ label: "Filter by Branch", value: "Filter by Branch" }]}
            />
          </div>

          <div className="w-40">
            <Select
              value="Options"
              options={[
                { label: "Options", value: "Options" },
                { label: "POS", value: "POS" },
                { label: "Customer", value: "Customer" },
              ]}
            />
          </div>

          <button className="bg-yellow-400 text-black px-6 py-2 rounded-md font-medium">
            Clear
          </button>

        </div>

        {/* ================= TABLE ================= */}
        <div className="bg-white rounded-lg overflow-hidden">

          <div className="overflow-x-auto">

            <table className="min-w-[1800px] w-full text-sm text-black">

              <thead className="bg-yellow-400">
                <tr>
                  {[
                    "Sl. No.",
                    "Reservation Date",
                    "Time Slot",
                    "No. of Guests",
                    "Status",
                    "Floor / Area",
                    "Table No.",
                    "Customer Name",
                    "Phone Number",
                    "Email Address",
                    "Reservation Source",
                    "Created On",
                    "Modified On",
                    "Modified by",
                  ].map((h) => (
                    <th
                      key={h}
                      className="px-4 py-3 text-left whitespace-nowrap"
                    >
                      {h}
                    </th>
                  ))}
                </tr>
              </thead>

              <tbody>
                {reservations.length > 0 ? (
                  reservations.map((reservation, idx) => (
                    <tr key={reservation.id} className={idx % 2 === 1 ? "bg-[#FFFAEB]" : idx % 2 === 0 && idx > 0 ? "" : "border-b"}>
                      <td className="px-4 py-3">{idx + 1}</td>
                      <td className="px-4 py-3">{formatDate(reservation.date)}</td>
                      <td className="px-4 py-3">{reservation.startTime} - {reservation.endTime}</td>
                      <td className="px-4 py-3">{reservation.guestCount}</td>
                      <td className="px-4 py-3">
                        <span className={`${statusColors[reservation.status] || "bg-gray-200 text-gray-600"} px-3 py-1 rounded-md text-xs`}>
                          {reservation.status}
                        </span>
                      </td>
                      <td className="px-4 py-3">{reservation.table?.floor?.name || "-"}</td>
                      <td className="px-4 py-3">{reservation.table?.label || "-"}</td>
                      <td className="px-4 py-3">{reservation.customerName}</td>
                      <td className="px-4 py-3">{reservation.customerPhone}</td>
                      <td className="px-4 py-3">{reservation.customer?.email || "-"}</td>
                      <td className="px-4 py-3">-</td>
                      <td className="px-4 py-3">{formatDateTime(reservation.createdAt)}</td>
                      <td className="px-4 py-3">{formatDateTime(reservation.updatedAt)}</td>
                      <td className="px-4 py-3">-</td>
                    </tr>
                  ))
                ) : (
                  <tr>
                    <td colSpan={14} className="px-4 py-8 text-center text-gray-500">
                      No reservation data available
                    </td>
                  </tr>
                )}
              </tbody>
            </table>

          </div>
        </div>

        {/* ================= PAGINATION ================= */}
        <div className="flex justify-end mt-6">
          <Pagination />
        </div>

      </div>
  );
};

export default ReservationReport;
