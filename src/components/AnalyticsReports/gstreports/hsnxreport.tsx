...
import { getGSTHSN } from "../../../services/reportsService";
import type { GstHsnReport } from "../../../services/reportsService";

const Hsnxreport = () => {
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [reportData, setReportData] = useState<GstHsnReport | null>(null);
  ...
  const fetchGSTData = async () => {
    try {
      ...
      const response = await getGSTHSN(month, year);

      if (response.success && response.data) {
        setReportData(response.data);
      } else {
        setError(response.message || "Failed to load GST HSN data");
      }
    } catch (err) {
      setError("An error occurred while loading GST HSN data");
      console.error("Error fetching GST HSN:", err);
    } finally {
      setLoading(false);
    }
  };
      ...
      {reportData && (
        <div className="bg-white rounded-lg border p-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div>
              <p className="text-bb-textSoft">Total Quantity</p>
              <p className="font-semibold">{reportData.summary.totalQuantity}</p>
            </div>
            <div>
        ...
        <div className="bg-white rounded-xl border border-bb-border overflow-hidden">
          <div className="overflow-x-auto">
            <table className="min-w-[1000px] text-sm whitespace-nowrap">
              ...
              <tbody>
                {reportData.hsnSummary.map((hsnItem, index) => (
                  <tr key={index} className={index % 2 === 1 ? "bg-bb-hover" : "border-b"}>
                    <td className="px-4 py-3">{hsnItem.hsnCode}</td>
                    <td className="px-4 py-3">{hsnItem.description}</td>
                    <td className="px-4 py-3">{hsnItem.quantity}</td>
                    <td className="px-4 py-3">₹{hsnItem.taxableAmount?.toFixed(2)}</td>
                    <td className="px-4 py-3">₹{hsnItem.taxAmount?.toFixed(2)}</td>
                    <td className="px-4 py-3">₹{hsnItem.totalAmount?.toFixed(2)}</td>
                  </tr>
                ))}
