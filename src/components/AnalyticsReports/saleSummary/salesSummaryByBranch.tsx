...
export default function SalesSummaryByBranch() {
  ...
  }, [dateFilter, startDate, endDate, selectedBranchId]);
  ...
  const fetchSalesData = async () => {
    try {
      ...
      const response = await getSalesSummary(start, end, selectedBranchId || undefined);
...
function SectionRow({ title, amount, open, onClick }: any) {
  return (
    <div
      onClick={onClick}
      className="grid grid-cols-2 px-4 py-2 cursor-pointer bg-[#FFF3CD] font-medium"
    >
...
function SubRow({ label, value }: any) {
  return (
    <div className="grid grid-cols-2 px-6 sm:px-8 py-1.5">
        ...
        <div className="absolute z-30 mt-1 w-full bg-white border rounded-md shadow max-h-60 overflow-auto">
          {options.map((opt: string) => (
            <div
              ...
              className="px-3 py-2 text-sm hover:bg-[#FFFBEA]"
            >
              {opt}
            </div>
          ))}
