...
const initialData: FloorArea[] = [
  { id: 1, name: "Private Dining", staff: "Salman Khan", status: false },
  { id: 2, name: "Family Section", staff: "Aman", status: true },
  { id: 3, name: "Bar", staff: "Salman Khan", status: false },
  { id: 4, name: "Non-AC", staff: "Aman", status: true },
  { id: 5, name: "Outdoor Seating", staff: "Salman Khan", status: false },
];
...
export default function FloorAreaStep({ data, onChange }: Props) {
  ...
  const toggleStatus = (id: number) => {
    const updated = areas.map((a) => (a.id === id ? { ...a, status: !a.status } : a));
    syncFloors(updated);
  };
              ...
              {areas.map((area, index) => (
                ...
                >
                    ...
                    >
                      <span
                        className={`h-4 w-4 bg-white rounded-full transition transform ${
                          area.status ? "translate-x-5" : "translate-x-1"
                        }`}
                      />
                    </button>
                  </td>
                  ...
                  <td className="px-4 py-3 text-right relative">
                    <button
                      onClick={() =>
                        setOpenMenu(openMenu === area.id ? null : area.id)
                      }
