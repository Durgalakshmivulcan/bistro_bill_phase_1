...
import { deleteBlogTagApi, BlogTag } from "../../services/blogService";
...
const BlogTagsTable = ({ tags, onDeleted }: BlogTagsTableProps) => {
  const [openId, setOpenId] = useState<string | null>(null);
  const [showConfirm, setShowConfirm] = useState(false);
  const [showSuccess, setShowSuccess] = useState(false);
  const [deleteItem, setDeleteItem] = useState<BlogTag | null>(null);
  const [deleting, setDeleting] = useState(false);
  const [deleteError, setDeleteError] = useState("");

  const navigate = useNavigate();

  const handleDeleteClick = (item: BlogTag) => {
    setOpenId(null);
    setDeleteItem(item);
    setDeleteError("");
    setShowConfirm(true);
  ...
  const confirmDelete = async () => {
    ...
    try {
      setDeleting(true);
      setDeleteError("");
      const response = await deleteBlogTagApi(deleteItem.id);
      if (response.success) {
        setShowConfirm(false);
        setShowSuccess(true);
      } else {
        setDeleteError(response.error?.message || "Failed to delete tag");
      }
    } catch (err) {
                    ...
                    {openId === item.id && (
                      <div className="absolute right-4 top-8 w-36 bg-white border rounded-md shadow z-20">
                        <button
                          onClick={() =>
                            navigate(
                              `/blog-management/tags/${item.id}/view`
                            )
                          }
                        ...
                        <button
                          onClick={() =>
                            navigate(
                              `/blog-management/tags/${item.id}/edit`
                            )
                          }
                          className="w-full flex items-center gap-2 px-4 py-2 text-sm hover:bg-gray-100"
                        >
      ...
      {showSuccess && deleteItem && (
        ...
        >
          ...
          <p className="text-sm text-gray-600">
            Blog tag <span className="font-medium">{deleteItem.name}</span> has
